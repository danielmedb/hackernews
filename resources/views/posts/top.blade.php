...
<div class="row" style="background-color: #eaeaea;">
    ...
    <div class="under text-muted small pl-4"> 
        {{ $post->votes->count() }} By {{ $post->user->name }} | {{ $post->created_at->diffForHumans() }} | <a href="/post/{{$post->id}}">{{ $post->comments->count() }} comments</a>
